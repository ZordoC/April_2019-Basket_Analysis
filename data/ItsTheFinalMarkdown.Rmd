...
For the sake of time-saving I will read the already saved csv file,otherwise would take me 15 minuts for each knit, which for doing your report is very time consuming. 
</br>
</br>
```{r}

Business2Business <- read.csv(file = "/home/zordo/Documents/Ubiqum/R-M2Task4/RM2T4/data/B2b4real", header = TRUE)
...
Now we just need to transform our dataframes into transactional objects and add a category level
</br>
</br>
```{r}
...
Now we just add new a level in itemInfo, called itemcategory.
</br>
</br>
```{r}

trans <- Business2BusinessTransactionObj
...
```{r}
Rules<- apriori (trans, parameter = list(supp = 0.0001, conf = 0.5,  minlen=2))
